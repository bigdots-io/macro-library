...
class CounterMacro extends Macro {
  ...
  start() {
    ...
    var textArea = new TypeWriter({
      font: 'system-16',
      alignment: 'right',
      startingColumn: this.dimensions.width - 2,
      startingRow: 1,
      hex: countColor
    });
    ...
    var makeRequest = function() {
      request.get({url: url, timeout: 10000}, function(error, response, body) {
        if(error) {
          textArea.write("failed", function(coordinates) {
            coordinates.forEach(function(coordinate) {
              onPixelChange(coordinate.y, coordinate.x, coordinate.hex || countColor);
            });
          });
        } else {
          ...
          textArea.write(commafy(totalLikes), function(coordinates) {
            coordinates.forEach(function(coordinate) {
              onPixelChange(coordinate.y, coordinate.x, coordinate.hex || countColor);
            });
          });
