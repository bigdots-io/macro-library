...
class HeartCounterMacro extends Macro {
  ...
  start() {
    ...
    var loadingBar = new LoadingBar(refreshInterval, {
      height: height,
      width: width,
    }, this.callbacks);
...
class LoadingBar {
  constructor(interval, dimensions, callbacks) {
    this.height = dimensions.height;
    this.width = dimensions.width;
    this.callbacks = callbacks;

    this.speed = (interval / this.width) * 1000;
  }
  ...
  start() {
    ...
    this.interval = setInterval(() => {
      this.callbacks.onPixelChange(this.height - 1, loadingPosition, '#999999');
      loadingPosition += 1;
    }, this.speed);
  }
  ...
  reset() {
    ...
    [...Array(this.width).keys()].forEach((x) => {
      this.callbacks.onPixelChange(this.height, x, '#000000');
    });
  }
}
